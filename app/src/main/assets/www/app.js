...
function _fsApplySlot(el, slot, visible) {
    ...
    // 포커스 라인(d===0)엔 약간의 오버슈트 스케일을 얹어 관성 스프링 느낌을 강화
    const bounce = slot.d === 0 ? 1.02 : 1;
    el.style.transform  = `scaleX(${slot.sc * bounce}) scaleY(${slot.sy * bounce}) translateY(${slot.dy}px)`;
}
...
function _highlightFsLine(idx) {
    ...
    inner.style.transition = `transform 0.58s var(--lyric-spring-move, cubic-bezier(0.22,1.14,0.3,1.02))`;
